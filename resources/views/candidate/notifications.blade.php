...
@section('content')
<div class="page-header"><div><h1>Notifications</h1><p>Updates on your applications and assessments</p></div>
...
async function load() {
    try {
        const data = await THR.api('/candidate/notifications');
        ...
        document.querySelectorAll('.read-btn').forEach(b => b.addEventListener('click', async () => {
            try { await THR.api(`/candidate/notifications/${b.dataset.id}/read`, { method: 'POST' }); load(); } catch(e){THR.toast(e.message,'danger');}
        }));
    } catch (e) { THR.toast(e.message, 'danger'); }
}
load();
document.getElementById('markAll').addEventListener('click', async () => {
    try { await THR.api('/candidate/notifications/read-all', { method: 'POST' }); THR.toast('All marked read','success'); load(); }
    catch (e) { THR.toast(e.message, 'danger'); }
});
</script>
@endpush
